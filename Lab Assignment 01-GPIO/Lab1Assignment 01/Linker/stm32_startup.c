...
/* ─── Tipo para entradas del vector table ─── */
...
{
    /* 1. Habilitar FPU (CP10 y CP11 con acceso completo) */
    FPU_CPACR |= (0xFU << 20);

    /* 2. Calcular tamaños de .data y .bss en palabras de 32 bits */
    uint32_t data_size = ((uint32_t)&_edata - (uint32_t)&_sdata) / 4;
    uint32_t bss_size  = ((uint32_t)&_ebss  - (uint32_t)&_sbss)  / 4;

    /* 3. Copiar .data de FLASH → SRAM */
    uint32_t *src  = (uint32_t *)&_etext;
    uint32_t *dest = (uint32_t *)&_sdata;
    for (uint32_t i = 0; i < data_size; i++)
        *dest++ = *src++;

    /* 4. Inicializar .bss a cero */
    dest = (uint32_t *)&_sbss;
    for (uint32_t i = 0; i < bss_size; i++)
        *dest++ = 0;

    /* 5. Llamar a main() via puntero a función para que GCC
          genere un BLX correcto en modo Thumb sin relocation
          problemática */
